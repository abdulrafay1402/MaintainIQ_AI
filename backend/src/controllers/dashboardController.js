...
const getAdminDashboard = asyncHandler(async (req, res) => {
  const [
    ...
    Issue.aggregate([{ $group: { _id: '$category', count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 6 }]),
  ]);
...
const getStudentDashboard = asyncHandler(async (req, res) => {
  ...
  const [total, pending, inProgress, completed, recentComplaints] = await Promise.all([
    Issue.countDocuments(filter),
    Issue.countDocuments({ ...filter, status: { $in: ['Reported', 'Assigned'] } }),
    Issue.countDocuments({ ...filter, status: { $in: ['Inspection Started', 'Maintenance In Progress', 'Waiting for Parts'] } }),
    Issue.countDocuments({ ...filter, status: { $in: ['Resolved', 'Verified', 'Closed'] } }),
    Issue.find(filter)
...
const getTechnicianDashboard = asyncHandler(async (req, res) => {
  ...
  const [assigned, pending, inProgress, completed, recentTasks] = await Promise.all([
    Issue.countDocuments(filter),
...
module.exports = { getAdminDashboard, getStudentDashboard, getTechnicianDashboard };
